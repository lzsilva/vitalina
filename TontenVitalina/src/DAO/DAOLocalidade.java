...
public class DAOLocalidade implements IDAO<Localidade> {
	...
	public ArrayList<Localidade> filtrar(String nome, String tipo) {
		// TODO Auto-generated method stub			
		boolean temNome = (nome!=null && !((nome.replace(" ", "")).isEmpty()));
		boolean temTipo = (tipo != null && !tipo.equalsIgnoreCase("default"));
		
		if (!temNome && !temTipo)			
			return this.buscar();	
		
		String query = "SELECT * FROM localidade WHERE ";
		
		if (temNome)
			query += "nomeLocalidade like ? ";
